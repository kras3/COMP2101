...
v4ip[0]=`ifconfig ${interfaces[0]}|grep "inet addr:"|sed -e 's/.*addr://' -e 's/ .*//'`
v4name[0]=$(getent hosts ${v4ip[0]}|sed -e 's/.*  *//')
v4ip[1]=`ifconfig ${interfaces[1]}|grep "inet addr:"|sed -e 's/.*addr://' -e 's/ .*//'`
v4name[1]=$(getent hosts ${v4ip[1]}|sed -e 's/.*  *//')
v6ip[0]=`ifconfig ${interfaces[0]}|grep "inet6 addr:"|sed -e 's/.*addr: //' -e 's,/.*,,'`
v6name[0]=$(getent hosts ${v6ip[0]}|sed -e 's/.*  *//')
v6ip[1]=`ifconfig ${interfaces[1]}|grep "inet6 addr:"|sed -e 's/.*addr: //' -e 's,/.*,,'`
